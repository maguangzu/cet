...
<script>
	...
	{
		document.forms[0].action="teamsl.do?page="+num;
...
<table width="85%" cellpadding="0" cellspafindmyactivecing="0" border="1">
	...
	<c:forEach items="${requestScope.allteampage.list}" var="at" varStatus="aa">
		<tr>
			<td>${aa.count}</td>		
			<td>${at.teamid}</td>
			<td>${at.teamname}</td>
			<td>${at.teamusernum}</td>
			<td>${at.teamusername}</td>
			<td>${at.teammadetime}</td>
			<td>${at.teamdesc}</td>
			<td>${at.teamzhtai}</td>
			<td><a href="teamsh.do?teamid=${at.teamid}">通过</a></td>
		</tr>
	</c:forEach>
	<tr style="text-align:center">
		<td colspan="9">
		<c:if test="${allteampage.currentPage != 1 }">
			<a onclick="goToPage(${allteampage.currentPage-1 })">上一页</a>
		</c:if>
		<c:forEach begin="1" end="${allteampage.totalPage }" varStatus="bb">
			<c:choose>
				<c:when test="${bb.index==allteampage.currentPage }">
		...
		<c:if test="${allteampage.currentPage!=allteampage.totalPage }">
			<a onclick="goToPage(${allteampage.currentPage+1})">下一页</a>
		</c:if>
		</td>
	</tr>
</table>
